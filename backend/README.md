...
./run_backend.sh test
```
Tests run with coverage enforced at the configured threshold. Use `config.yml` to raise the bar (e.g., to 100%) or to enable linting on CI/local runs. Current coverage is 100%.
